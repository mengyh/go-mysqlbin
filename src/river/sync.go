...
func (r *River) syncLoop() {
	...
	for {
		...
		case v := <-r.syncCh:
			...
			case posSaver:
				now := time.Now()
				if v.force || now.Sub(lastSavedTime) > 1*time.Second {
					lastSavedTime = now
					needFlush = true
					needSavePos = true
					pos = v.pos
...
func (r *River) runsql(dosql string){
	...
	cn, err := sql.Open("mysql", r.c.MyUser+":"+r.c.MyPassword+"@tcp("+r.c.MyAddr+")/"+sdatabase+"?charset=utf8")
	if err != nil {
		log.Errorf("err %v ", err)
		return
	}
	r.rdosql=r.rdosql+1
	_,err = cn.Query(dosql)
	r.rdosql=r.rdosql-1
	if err != nil {
