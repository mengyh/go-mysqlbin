...
func (r *River) newRule(schema, table string) error {
	...
	if _, ok := r.rules[key]; ok {
		return errors.Errorf("duplicate source %s, %s defined in config", schema, table,key)
	}
...
func (r *River) parseSource() (map[string][]string, error) {
	...
	for _, s := range r.c.Sources {
		...
		if len(s.Tables)>0{
			...
			//all tables
			tables := []string{}
			sql := fmt.Sprintf(`SELECT table_name FROM information_schema.tables WHERE table_schema = "%s";`,s.Schema)
			res, err := r.canal.Execute(sql)
			if err != nil {
				return nil, errors.Trace(err)
			}
			...
			wildTables[ruleKey(s.Schema, "*")] = tables
		}
	}
